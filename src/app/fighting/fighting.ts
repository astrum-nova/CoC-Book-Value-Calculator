...
export class Fighting {
  ...
  filterTroops(): void {
    ...
    this.playerData?.units?.forEach(unit => {
      ...
      if (this.playerData!.buildings[0].lvl! >= currentTroop.levels[levelIndex].required_lab_level) {
        if (!max && currentTroop.levels[levelIndex + 1].upgrade_time > this.maxTime) this.maxTime = currentTroop.levels[levelIndex + 1].upgrade_time;
        this.troops.push({
          name: currentTroop.name,
          level: currentTroop.levels[levelIndex].level,
          next: max ? -1 : currentTroop.levels[levelIndex].level + 1,
          time: max ? -1 : currentTroop.levels[levelIndex].upgrade_time,
        })
      }
    })
  }
  filterSieges(): void {
    ...
    this.playerData?.siege_machines?.forEach(siege => {
      ...
      if (this.playerData!.buildings[0].lvl! >= currentSiege.levels[levelIndex].required_lab_level) {
        if (!max && currentSiege.levels[levelIndex + 1].upgrade_time > this.maxTime) this.maxTime = currentSiege.levels[levelIndex + 1].upgrade_time;
        this.troops.push({
          name: currentSiege.name,
          level: currentSiege.levels[levelIndex].level,
          next: max ? -1 : currentSiege.levels[levelIndex].level + 1,
          time: max ? -1 : currentSiege.levels[levelIndex].upgrade_time,
        })
      }
    })
  }
